...
<p>A reproducible workflow from <strong>~700K raw array genotypes</strong> to <strong>40+ million imputed variants</strong> across multiple batches with <strong>2,113 samples</strong> ready for genome-wide association testing.</p>
...
2. **Merge:** harmonize chromosomes and variant IDs across different batches (cohort-0*) and merge by chromosome.
...
**Required software:** [PLINK 2](https://www.cog-genomics.org/plink/2.0/), [PLINK 1.9](https://www.cog-genomics.org/plink/1.9/), [bcftools](https://samtools.github.io/bcftools/), Bash, R, [UCSC liftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver) or [LocusLift](https://github.com/BioInUmer/LocusLift), [McCarthy tools](https://www.chg.ox.ac.uk/~wrayner/tools/), [aria2c](https://aria2.github.io/).
...
| Step | Action | Outcome |
|------|--------|---------|
| Sample exclusions | Remove microarray controls (`NA`s) and duplicate IID | 7 samples removed (6 controls + 1 duplicate) |
| Sex updates | Fill missing sex information from `file.csv` clinical data | Missing values updated |
| Variant IDs | Standardize IDs to `chr<N>:<pos>:<REF>:<ALT>` | Consistent variant naming |
| Conversion | Convert PLINK1 datasets to PLINK2 | PLINK2 datasets ready for QC |

**Variant Quality Control (SNP QC)**

| Filter | Criteria |
|--------|-----------|
| Chromosome | Retain autosomes (Chr 1-22) and Chr X |
| Alleles | Retain strictly biallelic variants |
| Duplicates | Exclude mismatched duplicate IDs |
| Missingness | Exclude variants with >2% missingness |
| Minor allele frequency | Exclude very rare or monomorphic variants with MAF <0.1% |
| Hardy-Weinberg equilibrium | Exclude variants with p < 1×10⁻⁶ |

*Result:* 730,059 initial variants reduced to 532,037 high-quality variants.

**Sample Quality Control**

| Check | Criteria | Outcome |
|-------|-----------|---------|
| Missingness | Exclude samples with >5% missingness | None removed |
| Heterozygosity | Exclude F coefficients outside the Bonferroni-adjusted `µ ± 4.17·SD` range | 5 samples removed |
| Sex concordance | chrX F-statistic: `F < 0.2` (female), `F > 0.8` (male) | 1 mismatch removed; 8 missing values updated |
| Ancestry (PCA) | project on 1000 Genomes and exclude outliers with Mahalanobis distance >9.21 (χ², 99% CI) | 14 samples removed |

*Result:* 336 initial samples cleaned to 310 samples.

**Final Preparation & McCarthy QC**

| Step | Action |
|------|--------|
| Monomorphic check | Prune variants that became monomorphic after sample removal |
| McCarthy QC (`HRC.pl`) | Validate strand, position, ambiguity, and REF/ALT alleles against imputation reference panel |
...
|--------|-----------|-----------|
| Chromosome scope | chr 1–22, X only | Remove chr 0, Y, XY, MT |
| Biallelic only | `--min/max-alleles 2` | Imputation servers require biallelic loci |
| Duplicate variants | `--rm-dup exclude-mismatch` | Remove conflicting duplicate entries |
| Call-rate | `--geno 0.02` (< 98%) | Remove poorly genotyped variants |
| MAF | `--maf 1e-3` (< 0.1%) | Remove very rare and monomorphic variants |
| HWE | `--hwe 1e-6` | Remove variants deviating from equilibrium |
...
| Filter | Threshold |
|--------|-----------|
| Call-rate | `--mind 0.05` (< 95%) |
| Heterozygosity | µ ± k·SD (Bonferroni-adjusted z-score) |
| Sex discrepancies | chrX F-statistic thresholds (F < 0.2 female, F > 0.8 male) |
| Ancestry outliers | PCA against 1000 Genomes reference |
...
The 1000GP PLINK files use GRCh37. Since our dataset has ~510k variants versus ~83M in 1000GP, it is far more efficient to liftover our data than the reference panel.

::: resource-callout
[LocusLift](https://github.com/BioInUmer/locuslift){target="_blank"}

This is a script I developed to automate liftover for PLINK and VCF files to any build.


**In a nutshell...**

The repo provides all the files needed to run the liftover process (chains, scripts, software, etc.). The script verifies before lifting, so you don't accidentally lift to the wrong build and provides a detailed log of the process.
:::

Below is the description of the manual UCSC `liftOver` approach that was originally used for `cohort-05`. The same steps are automated by [LocusLift](https://github.com/BioInUmer/locuslift){target="_blank"}.
...
The `.eigenvec` sample coordinate file is not used; both datasets are projected from scratch using the allele weights.
:::


#### Project Both Datasets

Both datasets are projected using the **same** 1000GP allele weights and allele frequencies. This is like a "double-projection", which ensures that both datasets (Cohort 05 and 1000G) are on the same scale without requiring post-hoc axis alignment.
...
PC1 vs PC2 and PC1 vs PC3 are plotted to visualize cohort ancestry relative to 1000GP super-populations.
...
This repo explains the outlier detection pipeline & rationale, and includes the scripts for making all the PCA plots.

**Brief Overview of PCAutliers pipeline:**  
Once our dataset is projected onto the 1000GP PCA space, we determine the ancestry of our samples (European). To keep homogeneity in our cohort, we aim to identify genetic outliers relative to our sample set, rather than to an ancestry of 1000GP. To do so, we use **Mahalanobis distance** to account for the elliptical shape of the PCA clusters. 

We calculate the distance of each sample from the cohort's centroid and exclude outliers using a highly permissive, threshold (Interquartile Range):

\begin{equation}
\text{Threshold} = Q_3 + 9 \times \text{IQR}
\end{equation}

Where $Q_3$ is the third quartile and $\text{IQR}$ is the interquartile range ($Q_3 - Q_1$). This approach ensures that only true, extreme outliers are removed.
:::

Below is the description of an alternative approach that was originally used for `cohort-05` during its initial quality control. While less robust than the IQR method, it serves as the historical record for this batch. Note that after all study batches were merged, the combined dataset underwent a final outlier filtering step using the full [PCAutliers](https://bioinumer.github.io/PCAutliers/){target="_blank"} pipeline.

**χ²-based Threshold**  
The squared Mahalanobis distance follows a **χ² distribution** with degrees of freedom (df) equal to the number of principal components used ($\text{df} = 2$). A **99% confidence cutoff** ($\chi^2 = 9.21$) was applied. Any samples exceeding this critical value were classified as non-European and excluded.
...
```bash
# Execute within cohort-N/vcf-files
for i in {1..23}; do
